...
function parseFunctionArgList(stream, shuntingYard) {
  ...
  withinSentinel(shuntingYard, function () {
    ...
    while (true) {
      ...
      // Track stack depth to detect empty arguments
      const stackDepthBefore = shuntingYard.operands.items.length;
      parseExpression(stream, shuntingYard);
      const stackDepthAfter = shuntingYard.operands.items.length;

      // If parseExpression didn't push anything, we have an empty argument
      if (stackDepthBefore === stackDepthAfter) {
        shuntingYard.operands.push(undefined);
      }
      arity += 1;
